# define namelist for community GSI release version 3.5

export comgsi_namelist="

 &SETUP
   miter=@outloop,niter(1)=@innerloop,niter(2)=@innerloop,
   write_diag(1)=.true.,write_diag(2)=.false.,write_diag(3)=.true.,
   gencode=78,qoption=2,
   factqmin=0.0,factqmax=0.0,
   iguess=-1,
   oneobtest=.@single_obs.,retrieval=.false.,
   nhr_assimilation=@cycle_interval,l_foto=.false.,
